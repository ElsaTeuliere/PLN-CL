...
F_Vec2Sym <- function(A.vec, diag=F){
   ...
   if(diag){n = (-1+sqrt(1+8*length(A.vec)))/2}
   A.mat = matrix(0, n, n)
   A.mat[lower.tri(A.mat, diag=diag)] = A.vec
   A.mat = A.mat + t(A.mat)
   if (diag){diag(A.mat) = diag(A.mat)/2}
   return(A.mat)
}

##################################################################
F_Sym2Vec <- function(A.mat, diag=F){
   # Makes a vector from the lower triangular par of a symmetric matrix
   return(A.mat[lower.tri(A.mat, diag=diag)])
}
