...
\\ 
In our case, the likelihood function is costly to calculate, because the dependency structure in the data implies to calculate integrals over $\mathbb{R}^n$. Since the distribution implies a latent variable, it can be tempting to estimate the parameters by Expectation-Minimization (EM). In the case of the Poisson log-normale distribution, there is no closed expression of the a posteriori distribution of $Z$ known $Y$ \cite{Karlis2005}. In this case previous studies proposed to perform estimations of the density. Other authors \cite{chiquet2017variational} proposed to estimate the parameters by a Variational Expectaition-Minimization (VEM) algorithm. However, there exist no  asymptotical guaranties on the consistence or normality of this estimator.
Several approached have been proposed in order to symplify the likeiyhood function in the case of complex dependencies. We propose to use the composite likelihood approach (see\cite{varin2011overview}, \cite{pedeli2018pairwise} and \cite{varin2008composite}).\\
\\
...
Let $\Theta$ be an euclidian space. For each $\theta \in \Theta$, let $\theta \mapsto \Psi_\theta (x)$ be a $\mathcal{C}^2$ function with respect to the variable $x$. Suppose that there exists $\theta_0$, such that $\mathrm{P} \Psi_{\theta_0} =0 $ and $\mathrm{P} \parallel \Psi_{\theta_0} \parallel^2 < \infty$. Indeed, we suppose that the matrix $\mathrm{P} \dot{\Psi}_{\theta_0}$ exists and is non-singular. Assume that the second-order partial derivatives are dominated by a fixed integrable function in the variable  $\ddot{\Psi}$ for every $\theta$ in a neiborhood of $\theta_0$. Consider a consistent estimator sequence $\widehat{\theta}_N$ such that $\Psi_N(\widehat{\theta}_N)=0$, the sequence $\sqrt{n}(\widehat{\theta}_N - \theta_0)$ converges in law to $\mathcal{N}(0,(\mathrm{P} \dot{\Psi}_{\theta_0})^{-1} \mathrm{P} \Psi_{\theta_0}  \Psi_{\theta_0}^T (\mathrm{P} \dot{\Psi}_{\theta_0})^{-1}))$.
\end{theorem}
...
\end{align*}
We think that also in this case, the estimator constructed by maximizing the pairwise composite likelihood over the pairs is a consistent and assymptotically normal estimator of $\beta$ and $\Gamma$. We propose to investigate also the sparse composite likelihood decribed in \ref{sparse}. We can generalize this model to $n \leq 1$. The matrix $\Sigma$ is then considered diagonal.\\
\\
